...
def parse_arguments(argv):
    ...
    parser.add_argument(
        ...
        nargs='*',
        metavar='arg',
        help="the arguments of the given command"
    )
    parser.add_argument(
        '--version',
        action='version',
...
def create_bind_from_string(volume):
    ...
    if ':' in destination:
        ...
        if readwrite == 'ro':
            ...
            raise argparse.ArgumentError('For creating volumes use the "ro", "rw" labels.')

    return BindMount(Path(source), Path(destination), readonly)


def run_container(root_dir, bind_mounts, isolate_networking, hostname, cmd, args):
...
def main(argv=sys.argv):
    ...
    if args.persistent:
        run_container(args.root_dir, args.volume, args.isolate_networking, args.hostname, args.cmd, args.args)
    else:
        with tempfile.TemporaryDirectory(
            ...
            run_container(overlay_mounted, args.volume, args.isolate_networking, args.hostname, args.cmd, args.args)
